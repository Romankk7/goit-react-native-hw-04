...
const RegistrationScreen = ({ navigation }) => {
   ...
   const register =()=> {
    if (!login || !mail || !password) { alert("Заполните поля для регистрации"); return }
   ...
   return (
    ...
    <KeyboardAvoidingView behavior={Platform.OS == "ios" ? "padding" : "height"} style={ styles.containerKeyB } >
      <View style={ styles.container }>
        ...
        <TextInput style={ styles.inputLogin } placeholder="Логин" inputMode="text" value={ login } onChangeText={handleLogin}/>
        <TextInput style={ styles.inputMailPassw } placeholder="Адрес электронной почты" inputMode="email" value={ mail }  onChangeText={handleMail}/>
        <TextInput style={ styles.inputMailPassw } placeholder="Пароль" secureTextEntry={showPassword} value={ password }  onChangeText={handlePassword}/>
        
        <TouchableOpacity style={ styles.passwShow } activeOpacity={0.5} onPress={passwShow}>
             <Text style={ styles.passwShowText }>{showPassword
          ? `Показать`
          : "Скрыть"}</Text>
        </TouchableOpacity>  
